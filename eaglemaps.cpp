...
void EagleMaps::initializeMap() {
    ...
    campusMap.addEdge(mccarty, wilber, 0.4, 8.0);
    campusMap.addEdge(mccarty, hillcrest, 0.2, 4.0);
    campusMap.addEdge(mccarty, thad, 0.3, 6.0);
    campusMap.addEdge(mccarty, library, 0.5, 10.0);
    campusMap.addEdge(mccarty, mathZone, 0.5, 10.0);
    campusMap.addEdge(mccarty, liberalArts, 0.6, 12.0);
    campusMap.addEdge(mccarty, asbury, 0.7, 14.0);
    campusMap.addEdge(mccarty, lucas, 0.5, 10.0);
    campusMap.addEdge(mccarty, elizabeth, 0.7, 14.0);
    campusMap.addEdge(mccarty, honorBusiness, 0.4, 8.0);
    ...
    campusMap.addEdge(library, mathZone, 0.05, 0.30);
}
...
void EagleMaps::displayMenu() {
    ...
    cout << "7. Manage Building Aliases\n";
    cout << "8. Save or Load Campus Map\n";
    cout << "0. Exit\n";
...
void EagleMaps::manageAliases() {
    cout << "\n===== MANAGE BUILDING ALIASES =====\n";
    cout << "1. View Aliases for a Building\n";
    cout << "2. Add Aliases to a Building\n";
    ...
    if (choice == 1) {
        cout << "Select a building to view aliases:\n";
        int idx = getBuildingSelection();
        string name = buildingNames[idx];
        campusMap.printAliasesForBuilding(name);
    }
    else if (choice == 2) {
        cout << "Select a building to add aliases to:\n";
        int idx = getBuildingSelection();
        string name = buildingNames[idx];
        campusMap.addAliasesToBuilding(name);
    }
    else if (choice == 3) {
        return;
    }
...
// Save or load campus map
void EagleMaps::saveLoadMap() {
    cout << "\n===== SAVE OR LOAD CAMPUS MAP =====\n";
    cout << "1. Save Current Map\n";
    cout << "2. Load Map from File\n";
    cout << "3. Return to Main Menu\n";
    cout << "Enter your choice: ";
    ...
    if (choice == 1) {
        string filename;
        cout << "Enter filename to save as (e.g., campus_map.txt): ";
        cin.ignore();
        getline(cin, filename);
        campusMap.saveToFile(filename);
    }
    else if (choice == 2) {
        string filename;
        ...
        vector<string> oldBuildingNames = buildingNames;
        
        campusMap.loadFromFile(filename);
        ...
        cout << "Building list updated with " << buildingNames.size() << " buildings.\n";
    }
    else if (choice == 3) {
        return;
    }
